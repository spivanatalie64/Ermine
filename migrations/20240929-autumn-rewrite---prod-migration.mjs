...
// NODE_EXTRA_CA_CERTS=~/projects/ermin-admin-panel/ermin.crt node index.mjs
// NODE_EXTRA_CA_CERTS=/cwd/ermin.crt node /cwd/index.mjs
  ...
  await Promise.all(
    new Array(workerCount).fill(0).map(async (_) => {
      while (true) {
        ...
        const metaUseInfo = await determinUploaderIdAndUse(doc, dir, i);
        if (!metaUseInfo) {
          if (USE_CACHE) {
        ...
        try {
          ...
          if (err.code === "ENOENT") {
            ...
            await mongo.db("ermin").collection("logs").insertOne({
              type: "missingFile",
